...
class _CashDepositScreenState extends State<CashDepositScreen> {
  ...
  @override
  void dispose() {
    for (final c in [_depositDateCtrl, _bankAccountNameCtrl, _depositSlipNoCtrl, _fromAccountNameCtrl, _amountCtrl, _narrationCtrl]) { c.dispose(); }
    super.dispose();
  }
  ...
  void _clearForm() {
    ...
    _bankAccountNameCtrl.clear(); _depositSlipNoCtrl.clear(); _fromAccountNameCtrl.clear(); _amountCtrl.text = '0'; _narrationCtrl.clear();
  }
  ...
  void _loadFromModel(BankDepositModel m) {
    ...
    _depositDateCtrl.text = m.depositDate; _bankAccountNameCtrl.text = m.bankAccountName; _depositSlipNoCtrl.text = m.depositSlipNo; _fromAccountNameCtrl.text = m.fromAccountName; _amountCtrl.text = m.amount.toStringAsFixed(2); _narrationCtrl.text = m.narration;
  }

  Map<String, dynamic> _buildPayload() => {
    'depositType': 'cash', 'depositDate': _depositDateCtrl.text, 'bankAccountId': _bankAccountId, 'bankAccountName': _bankAccountNameCtrl.text, 'depositSlipNo': _depositSlipNoCtrl.text, 'fromAccountId': _fromAccountId, 'fromAccountName': _fromAccountNameCtrl.text, 'amount': double.tryParse(_amountCtrl.text) ?? 0, 'narration': _narrationCtrl.text, 'status': 'saved',
  };
  ...
  Future<void> _openRecords() async {
    ...
    final record = await RecordBrowserDialog.show<BankDepositModel>(
      ...
      getSubtitle: (m) => '${m.depositDate.isNotEmpty ? m.depositDate.substring(0,10) : "?"}  |  Rs ${m.amount.toStringAsFixed(0)}',
    );
