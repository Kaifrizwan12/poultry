...
import '../services/stock_expiry_service.dart';

class StockExpiryController extends ChangeNotifier {
  StockExpiryController() : _service = StockExpiryService() {
    OfflineSyncService.instance.addSyncListener(_onSyncComplete);
    OfflineSyncService.instance.addTempIdListener('stock-expiries', _onTempIdReplaced);
  }

  final StockExpiryService _service;
  List<StockExpiryModel> _items = [];
  bool _isLoading = false;
  String? _error;
  bool _isOfflineData = false;

  List<StockExpiryModel> get items  => _items;
  ...
  void dispose() {
    ...
    OfflineSyncService.instance.removeTempIdListener('stock-expiries', _onTempIdReplaced);
    super.dispose();
  }
  ...
  Future<StockExpiryModel> add(Map<String, dynamic> payload) async {
  ...
  void _onSyncComplete() {
    final cached = OfflineCacheService.readList(
        OfflineCacheService.invoicingListKey('stock-expiries'));
