...
class SalesReturnController extends ChangeNotifier {
  SalesReturnController() : _service = SalesReturnService() {
    OfflineSyncService.instance.addSyncListener(_onSyncComplete);
    OfflineSyncService.instance.addTempIdListener('sales-returns', _onTempIdReplaced);
  }
  ...
  List<SalesReturnModel> get items  => _items;
  ...
  void dispose() {
    ...
    OfflineSyncService.instance.removeTempIdListener('sales-returns', _onTempIdReplaced);
    super.dispose();
  }
  ...
  Future<SalesReturnModel> add(Map<String, dynamic> payload) async {
  ...
  void _onSyncComplete() {
    final cached = OfflineCacheService.readList(
        OfflineCacheService.invoicingListKey('sales-returns'));
