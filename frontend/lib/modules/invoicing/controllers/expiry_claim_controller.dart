...
import '../services/expiry_claim_service.dart';

class ExpiryClaimController extends ChangeNotifier {
  ExpiryClaimController() : _service = ExpiryClaimService() {
    OfflineSyncService.instance.addSyncListener(_onSyncComplete);
    OfflineSyncService.instance.addTempIdListener('expiry-claims', _onTempIdReplaced);
  }
  ...
  List<ExpiryClaimModel> get items  => _items;
  ...
  void dispose() {
    ...
    OfflineSyncService.instance.removeTempIdListener('expiry-claims', _onTempIdReplaced);
    super.dispose();
  }
  ...
  Future<ExpiryClaimModel> add(Map<String, dynamic> payload) async {
  ...
  void _onSyncComplete() {
    final cached = OfflineCacheService.readList(
        OfflineCacheService.invoicingListKey('expiry-claims'));
