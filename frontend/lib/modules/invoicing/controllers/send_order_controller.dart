...
class SendOrderController extends ChangeNotifier {
  SendOrderController() : _service = SendOrderService() {
    OfflineSyncService.instance.addSyncListener(_onSyncComplete);
    OfflineSyncService.instance.addTempIdListener('send-orders', _onTempIdReplaced);
  }

  final SendOrderService _service;
  List<SendOrderModel> _items = [];
  bool _isLoading = false;
  String? _error;
  bool _isOfflineData = false;

  List<SendOrderModel> get items  => _items;
  ...
  void dispose() {
    ...
    OfflineSyncService.instance.removeTempIdListener('send-orders', _onTempIdReplaced);
    super.dispose();
  }
  ...
  Future<SendOrderModel> add(Map<String, dynamic> payload) async {
  ...
  SendOrderModel? findById(String id) {
    try { return _items.firstWhere((i) => i.id == id || i.text('sendOrderId') == id); } catch (_) { return null; }
  }

  void _onSyncComplete() {
    final cached = OfflineCacheService.readList(
        OfflineCacheService.invoicingListKey('send-orders'));
