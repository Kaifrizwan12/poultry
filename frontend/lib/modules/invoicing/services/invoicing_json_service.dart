...
/// Base service for all invoicing CRUD entities. Handles both online and offline
/// paths transparently. All invoicing models extend [BaseSettingsModel].
class InvoicingJsonService<T> {
  InvoicingJsonService({
    required this.entityPath,
    required T Function(Map<String, dynamic>) parser,
  })  : _parser = parser,
        _api = ApiService();

  final String entityPath; // e.g. 'sales-invoices'
  final T Function(Map<String, dynamic>) _parser;
  final ApiService _api;

  String get _basePath => '/invoicing/$entityPath';
  ...
  static void _log(String method, String path,
    ...
    if (error != null) {
      debugPrint('[$ts] [INVOICING] ERROR $method $full — $error');
    } else if (detail != null) {
      debugPrint('[$ts] [INVOICING] $method $full — $detail');
    } else {
      debugPrint('[$ts] [INVOICING] $method $full');
    }
  }
  ...
  Future<List<T>> fetchAll({String? queryString}) async {
    ...
    final cacheKey = OfflineCacheService.invoicingListKey(entityPath,
  ...
  Future<T> fetchById(String id) async {
    ...
    final cacheKey = OfflineCacheService.invoicingItemKey(entityPath, id);
    _log('GET', path);
    try {
      final response = await _api.get(path, auth: true);
    ...
    try {
      ...
      if (item is BaseSettingsModel) {
        await OfflineCacheService.saveItem(
            OfflineCacheService.invoicingItemKey(entityPath, item.id),
            _toMap(item));
      }
  ...
  Future<T> update(String id, Map<String, dynamic> body) async {
    ...
    try {
      ...
      await OfflineCacheService.saveItem(
          OfflineCacheService.invoicingItemKey(entityPath, id), updated);
      ...
      await OfflineCacheService.saveItem(
          OfflineCacheService.invoicingItemKey(entityPath, id), fullMap);
  ...
  Future<void> delete(String id) async {
    ...
    try {
      ...
      await OfflineCacheService.removeItem(
          OfflineCacheService.invoicingItemKey(entityPath, id));
