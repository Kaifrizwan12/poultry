...
module.exports = {
  ...
  async sanitize({ uid, body, id }) {
    ...
    const purchaseInvoiceId = asNullableString(body.purchaseInvoiceId);
    if (returnType === 'with_invoice') {
      if (!purchaseInvoiceId) errors.push('purchaseInvoiceId is required for with_invoice return type');
      else await requireInvoicingRef(uid, 'purchaseInvoices', purchaseInvoiceId, 'purchaseInvoiceId', errors);
    }
