...
import '../services/recovery_receivable_wise_service.dart';

class RecoveryReceivableWiseController extends ChangeNotifier {
  RecoveryReceivableWiseController() : _service = RecoveryReceivableWiseService() {
    OfflineSyncService.instance.addSyncListener(_onSyncComplete);
    OfflineSyncService.instance.addTempIdListener('recovery-receivable-wise', _onTempIdReplaced);
  }
  ...
  List<RecoveryReceivableWiseModel> get items  => _items;
  ...
  void dispose() {
    ...
    OfflineSyncService.instance.removeTempIdListener('recovery-receivable-wise', _onTempIdReplaced);
    super.dispose();
  }
  ...
  Future<RecoveryReceivableWiseModel> add(Map<String, dynamic> payload) async {
  ...
  void _onSyncComplete() {
    final cached = OfflineCacheService.readList(
        OfflineCacheService.invoicingListKey('recovery-receivable-wise'));
