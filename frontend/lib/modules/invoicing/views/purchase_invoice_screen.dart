...
class _PurchaseInvoiceScreenState extends State<PurchaseInvoiceScreen> {
  ...
  final _entryDateCtrl = TextEditingController(text: DateTime.now().toIso8601String().substring(0, 10));
  final _vendorBillNoCtrl = TextEditingController();
  final _billDateCtrl = TextEditingController(text: DateTime.now().toIso8601String().substring(0, 10));
  ...
  @override
  void dispose() {
    for (final c in [_entryDateCtrl, _vendorBillNoCtrl, _billDateCtrl, _vendorNameCtrl, _cityCtrl, _disc2PercentCtrl, _fTaxCtrl, _totalSEDCtrl, _spcDiscCtrl, _prevCreditCtrl, _paidAmountCtrl]) {
      c.dispose();
    }
  ...
  void _clearForm() {
    setState(() { _currentId = null; _purchaseId = ''; _vendorId = ''; _linkedOrderId = ''; });
    _entryDateCtrl.text = DateTime.now().toIso8601String().substring(0, 10);
    _vendorBillNoCtrl.clear(); _billDateCtrl.text = DateTime.now().toIso8601String().substring(0, 10);
    _vendorNameCtrl.clear(); _cityCtrl.clear();
    for (final c in [_disc2PercentCtrl, _fTaxCtrl, _totalSEDCtrl, _spcDiscCtrl, _prevCreditCtrl, _paidAmountCtrl]) {
      c.text = '0';
    }
    _itemsNotifier.value = [];
  }

  void _loadFromModel(PurchaseInvoiceModel m) {
    setState(() { _currentId = m.id; _purchaseId = m.purchaseId; _vendorId = m.vendorId; _linkedOrderId = m.orderId; });
    _entryDateCtrl.text = m.entryDate; _vendorBillNoCtrl.text = m.vendorBillNo;
    _billDateCtrl.text = m.billDate; _vendorNameCtrl.text = m.vendorName; _cityCtrl.text = m.city;
  ...
  Map<String, double> _computeTotals(List<Map<String, dynamic>> items) {
    ...
    return { 'gross': gross, 'disc2Percent': disc2, 'discounts': discounts, 'invoiceValue': invoiceValue, 'salesTax': salesTax, 'fTax': fTax, 'totalSED': totalSED, 'spcDisc': spcDisc, 'netValue': netValue, 'totalPayable': totalPayable, 'paidAmount': paidAmount, 'remBalance': remBalance };
  }
  ...
  Map<String, dynamic> _buildPayload(String status) {
    ...
    return {
      'entryDate': _entryDateCtrl.text, 'vendorBillNo': _vendorBillNoCtrl.text,
      'billDate': _billDateCtrl.text, 'orderId': _linkedOrderId,
      'vendorId': _vendorId, 'vendorName': _vendorNameCtrl.text, 'city': _cityCtrl.text,
  ...
  Future<void> _save(String status) async {
    ...
    try {
      ...
      if (_currentId == null) {
        ...
        setState(() { _currentId = record.id; _purchaseId = record.purchaseId; });
      } else {
  ...
  Future<void> _remove() async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
      title: const Text('Confirm Delete'), content: const Text('Delete this purchase invoice?'),
      actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')), ElevatedButton(onPressed: () => Navigator.pop(context, true), style: ElevatedButton.styleFrom(backgroundColor: AppTheme.dangerText), child: const Text('Delete'))],
    ));
    if (confirmed == true) { await context.read<PurchaseInvoiceController>().deleteItem(_currentId!); _clearForm(); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Deleted'))); }
  }
  ...
  Future<void> _openRecords({bool pendingOnly = false}) async {
    ...
    if (!mounted) return;
    final record = await RecordBrowserDialog.show<PurchaseInvoiceModel>(
      context: context,
      records: pendingOnly ? ctrl.pendingItems : ctrl.items,
      title: 'Purchase Invoices',
      getBusinessId: (m) => m.purchaseId,
      getTitle: (m) => '${m.purchaseId}  •  ${m.vendorName}',
      getSubtitle: (m) => '${m.entryDate.substring(0, 10)}  |  ${m.status}  |  Rs ${m.netValue.toStringAsFixed(0)}',
      statusField: 'status',
    );
  ...
  Widget build(BuildContext context) {
    ...
    final orders = context.read<PurchaseOrderController>().items;
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  Text('Purchase Invoice', style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(width: 16),
                  if (_purchaseId.isNotEmpty)
                    Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: AppTheme.terra50, borderRadius: BorderRadius.circular(6), border: Border.all(color: AppTheme.terra200)),
                      child: Text('PI: $_purchaseId', style: const TextStyle(color: AppTheme.terra800, fontWeight: FontWeight.w600, fontSize: 13))),
                ]),
                const SizedBox(height: 16),
                Container(
                  decoration: AppTheme.cardDecor, padding: AppTheme.cardPadding,
                  child: Wrap(spacing: 12, runSpacing: 12, children: [
                    SizedBox(width: 160, child: TextFormField(controller: _entryDateCtrl, decoration: _dec('Entry Date'), readOnly: true, onTap: () async { final p = await showDatePicker(context: context, initialDate: DateTime.tryParse(_entryDateCtrl.text) ?? DateTime.now(), firstDate: DateTime(2000), lastDate: DateTime(2100)); if (p != null) setState(() => _entryDateCtrl.text = p.toIso8601String().substring(0, 10)); })),
                    SizedBox(width: 160, child: TextFormField(controller: _vendorBillNoCtrl, decoration: _dec('Vendor Bill No'))),
                    SizedBox(width: 160, child: TextFormField(controller: _billDateCtrl, decoration: _dec('Bill Date'), readOnly: true, onTap: () async { final p = await showDatePicker(context: context, initialDate: DateTime.tryParse(_billDateCtrl.text) ?? DateTime.now(), firstDate: DateTime(2000), lastDate: DateTime(2100)); if (p != null) setState(() => _billDateCtrl.text = p.toIso8601String().substring(0, 10)); })),
                    SizedBox(
                      width: 220,
                      child: DropdownButtonFormField<String>(
                        // value = Firestore doc ID (unique) — never use orderId string as value
                        value: _linkedOrderId.isEmpty ? null : _linkedOrderId,
                        decoration: _dec('Link to PO (optional)'), isExpanded: true,
                        items: [
                          const DropdownMenuItem<String>(value: '', child: Text('None')),
                          ...orders.map((o) => DropdownMenuItem<String>(
                            value: o.id,   // Firestore doc ID — always unique
                            child: Text(
                              o.orderId.isNotEmpty ? '${o.orderId}  •  ${o.vendorName}' : o.id,
                        ...
                        onChanged: (docId) {
                          ...
                          if (docId != null && docId.isNotEmpty) {
                            ...
                            _vendorNameCtrl.text = po.vendorName; _cityCtrl.text = po.city;
                    ...
                    SizedBox(
                      ...
                      child: DropdownButtonFormField<String>(
                        ...
                        onChanged: (val) { if (val == null) return; final v = vendors.firstWhere((x) => x.id == val); setState(() => _vendorId = val); _vendorNameCtrl.text = v.text('name'); _cityCtrl.text = v.text('city'); },
                      ),
                    ),
                    SizedBox(width: 140, child: TextFormField(controller: _cityCtrl, decoration: _dec('City'))),
                    ...
                    SizedBox(width: 100, child: TextFormField(controller: _fTaxCtrl, decoration: _dec('F.Tax'), keyboardType: TextInputType.number, onChanged: (_) => setState(() {}))),
                    SizedBox(width: 100, child: TextFormField(controller: _totalSEDCtrl, decoration: _dec('Total SED'), keyboardType: TextInputType.number, onChanged: (_) => setState(() {}))),
                    SizedBox(width: 100, child: TextFormField(controller: _spcDiscCtrl, decoration: _dec('Spc Disc'), keyboardType: TextInputType.number, onChanged: (_) => setState(() {}))),
                    ...
                    builder: (context, items, _) {
                      ...
                      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Container(decoration: AppTheme.cardDecor, child: SingleChildScrollView(scrollDirection: Axis.horizontal, child: DataTable(
                          ...
                          columns: const [DataColumn(label: Text('#')), DataColumn(label: Text('Product')), DataColumn(label: Text('Packing')), DataColumn(label: Text('Qty(P)')), DataColumn(label: Text('Qty(L)')), DataColumn(label: Text('Price')), DataColumn(label: Text('Disc%')), DataColumn(label: Text('Tax%')), DataColumn(label: Text('Gross')), DataColumn(label: Text('Val incST')), DataColumn(label: Text(''))],
                          rows: items.asMap().entries.map((e) { final idx = e.key; final it = e.value; return DataRow(
                            ...
                            cells: [DataCell(Text('${idx + 1}')), DataCell(Text(it['productName'] ?? '')), DataCell(Text(it['packingName'] ?? '')), DataCell(Text('${it['qtyPacks'] ?? ''}')), DataCell(Text('${it['qtyLoose'] ?? ''}')), DataCell(Text((it['price'] as num?)?.toStringAsFixed(2) ?? '0')), DataCell(Text((it['discPercent'] as num?)?.toStringAsFixed(2) ?? '0')), DataCell(Text((it['salesTaxPercent'] as num?)?.toStringAsFixed(2) ?? '0')), DataCell(Text((it['lineGross'] as num?)?.toStringAsFixed(2) ?? '0')), DataCell(Text((it['lineValueIncST'] as num?)?.toStringAsFixed(2) ?? '0')), DataCell(IconButton(icon: const Icon(Icons.delete_outline, size: 18, color: AppTheme.dangerText), onPressed: () { final u = [...items]; u.removeAt(idx); _itemsNotifier.value = u; }))],
                          ); }).toList(),
                        ))),
                        const SizedBox(height: 8),
                        InvoiceTotalsFooter(totals: _computeTotals(items), visibleKeys: const ['gross', 'discounts', 'invoiceValue', 'salesTax', 'fTax', 'totalSED', 'spcDisc', 'netValue', 'totalPayable', 'paidAmount', 'remBalance']),
                      ]);
                    },
                  ),
                ),
