...
import '../services/recovery_invoice_wise_service.dart';

class RecoveryInvoiceWiseController extends ChangeNotifier {
  RecoveryInvoiceWiseController() : _service = RecoveryInvoiceWiseService() {
    OfflineSyncService.instance.addSyncListener(_onSyncComplete);
    OfflineSyncService.instance.addTempIdListener('recovery-invoices-wise', _onTempIdReplaced);
  }

  final RecoveryInvoiceWiseService _service;
  List<RecoveryInvoiceWiseModel> _items = [];
  bool _isLoading = false;
  String? _error;
  bool _isOfflineData = false;

  List<RecoveryInvoiceWiseModel> get items  => _items;
  ...
  void dispose() {
    ...
    OfflineSyncService.instance.removeTempIdListener('recovery-invoices-wise', _onTempIdReplaced);
    super.dispose();
  }
  ...
  Future<RecoveryInvoiceWiseModel> add(Map<String, dynamic> payload) async {
  ...
  RecoveryInvoiceWiseModel? findById(String id) {
  ...
  void _onSyncComplete() {
    final cached = OfflineCacheService.readList(
        OfflineCacheService.invoicingListKey('recovery-invoices-wise'));
