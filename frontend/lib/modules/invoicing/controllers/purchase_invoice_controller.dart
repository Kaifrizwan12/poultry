...
class PurchaseInvoiceController extends ChangeNotifier {
  PurchaseInvoiceController() : _service = PurchaseInvoiceService() {
    OfflineSyncService.instance.addSyncListener(_onSyncComplete);
    OfflineSyncService.instance.addTempIdListener('purchase-invoices', _onTempIdReplaced);
  }
  ...
  Future<PurchaseInvoiceModel> add(Map<String, dynamic> payload) async {
  ...
  PurchaseInvoiceModel? findById(String id) {
    try { return _items.firstWhere((i) => i.id == id || i.text('purchaseId') == id); } catch (_) { return null; }
  }

  void _onSyncComplete() {
    final cached = OfflineCacheService.readList(
        OfflineCacheService.invoicingListKey('purchase-invoices'));
