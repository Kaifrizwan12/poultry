...
class _StockExpiryScreenState extends State<StockExpiryScreen> {
  ...
  void _clearForm() {
    setState(() { _currentId = null; _expiryId = ''; _lineProductId = null; _linePackingId = ''; _linePack = 1; _lineCost = 0; _lineValue = 0; });
  ...
  void _loadFromModel(StockExpiryModel m) {
    setState(() { _currentId = m.id; _expiryId = m.expiryId; });
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_currentId == null) { final r = await ctrl.add(_buildPayload()); setState(() { _currentId = r.id; _expiryId = r.expiryId; }); }
  ...
  Future<void> _remove() async {
    ...
    final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(title: const Text('Confirm Delete'), content: const Text('Delete this expiry record?'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')), ElevatedButton(onPressed: () => Navigator.pop(context, true), style: ElevatedButton.styleFrom(backgroundColor: AppTheme.dangerText), child: const Text('Delete'))]));
    if (ok == true) { await context.read<StockExpiryController>().deleteItem(_currentId!); _clearForm(); if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Deleted'))); }
  }
  ...
  Future<void> _openRecords() async {
    ...
    if (!mounted) return;
    final record = await RecordBrowserDialog.show<StockExpiryModel>(
      context: context,
      records: ctrl.items,
      title: 'Stock Expiries',
      getBusinessId: (m) => m.expiryId,
      getTitle: (m) => m.expiryId,
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Text('Stock Expiry Invoice', style: Theme.of(context).textTheme.titleLarge),
                const SizedBox(width: 16),
                if (_expiryId.isNotEmpty) Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: AppTheme.terra50, borderRadius: BorderRadius.circular(6), border: Border.all(color: AppTheme.terra200)), child: Text('Expiry ID: $_expiryId', style: const TextStyle(color: AppTheme.terra800, fontWeight: FontWeight.w600, fontSize: 13))),
              ]),
